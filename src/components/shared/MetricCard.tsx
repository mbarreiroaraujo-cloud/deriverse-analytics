...
export function MetricCard({ title, value, subtitle, icon, trend, trendValue, valueColor = 'default', delay = 0 }: MetricCardProps) {
  ...
  return (
    <div
      className={`bg-bg-secondary/80 border border-border/50 rounded-2xl p-3 sm:p-5 shadow-sm shadow-black/20 card-hover transition-all duration-500 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] sm:text-xs font-medium text-text-muted uppercase tracking-wider">{title}</span>
